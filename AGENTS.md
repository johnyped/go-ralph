...
This repo is `go-ralph` — a Go CLI that orchestrates `pi` coding sessions via `herdr`, with support for sequential and parallel execution.
...
internal/
  ...
  pipeline/   load/write .ralph/pipeline.yaml; ReadyIssues dependency resolver
  cmd/        cobra commands: init, run, status, reset, logs
```

## Key Decisions

- **Sequential or parallel** — controlled by `max_parallel` in config and presence of `pipeline.yaml`. Three execution modes: no pipeline (sequential for-loop), pipeline + `max_parallel=1` (sequential path order), pipeline + `max_parallel>1` (semaphore-bounded goroutines).
- **Pipeline file** — `.ralph/pipeline.yaml` defines named paths (ordered lists of issue IDs) and optional `depends_on` between paths. Generated by `go-ralph init`.
...
- **Configurable timeout** — `timeout_minutes` controls per-attempt wait time.
...
- **Stop on failure by default** — `stop_on_failure: true` in config; `--continue` flag overrides per run. In parallel mode, already-running goroutines are drained before exit.
- **Project-local only** — no global `~/.ralph/` config; everything under `.ralph/` in the project dir.
- **Per-issue model** — `IssueState.Model` is seeded from `default_model` on `init`. Non-empty value adds `--model <value>` to the pi invocation.

## Commands

```
go-ralph init   <project>  --dir <path>  [--parallel N]
...
```bash
pi --mode json [--model <model>] --name "ralph: <slug>" "$(cat promptFile)" \
  | tee <slug>.jsonl \
...
`--model` is omitted when `IssueState.Model` is empty. The renderer is defined as the `rendererScript` constant in `internal/pi/pi.go`.
...
```
.ralph/
  config.yaml          # ralph config (pi_path, max_parallel, default_model, …)
  pipeline.yaml        # path definitions and depends_on (generated by init)
  <project>.json       # issue state
